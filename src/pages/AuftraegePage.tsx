...
export default function AuftraegePage() {
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, fahrzeugeData, kundenData] = await Promise.all([
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('arbeitsbeschreibung')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('auftraege', 'arbeitsbeschreibung')}
                  {sortKey === 'arbeitsbeschreibung' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('wunschtermin')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('auftraege', 'wunschtermin')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('prioritaet')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('auftraege', 'prioritaet')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('bemerkungen_auftrag')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('auftraege', 'bemerkungen_auftrag')}
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/auftraege/${record.record_id}`); }}>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.arbeitsbeschreibung ?? '—'}</span></TableCell>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('auftraege', 'prioritaet', record.fields.prioritaet?.key) ?? record.fields.prioritaet?.label ?? '—'}</span></TableCell>
