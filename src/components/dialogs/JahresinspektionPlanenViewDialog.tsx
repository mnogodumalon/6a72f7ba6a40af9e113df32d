  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t('view_entity', { entity: appLabel('jahresinspektion_planen') })}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('jahresinspektion_planen', 'fahrzeug')}</Label>
            <p className="text-sm">{getFahrzeugeDisplayName(record.fields.fahrzeug)}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('jahresinspektion_planen', 'wunschtermin_inspektion')}</Label>
            <p className="text-sm">{formatDate(record.fields.wunschtermin_inspektion)}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('jahresinspektion_planen', 'arbeitsbeschreibung_inspektion')}</Label>
            <p className="text-sm whitespace-pre-wrap">{record.fields.arbeitsbeschreibung_inspektion ?? '—'}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">{fieldLabel('jahresinspektion_planen', 'bemerkungen_inspektion')}</Label>
