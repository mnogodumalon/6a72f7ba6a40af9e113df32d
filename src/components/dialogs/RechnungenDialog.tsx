...
export type RechnungenDialogDefaults = Omit<Rechnungen['fields'], 'mwst_satz' | 'status_rechnung'> & {
    mwst_satz?: LookupValue | string;
    status_rechnung?: LookupValue | string;
  };
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  mwst_satz: LOOKUP_OPTIONS['rechnungen']?.['mwst_satz'] ?? [],
  status_rechnung: LOOKUP_OPTIONS['rechnungen']?.['status_rechnung'] ?? [],
};
...
export function RechnungenDialog({ open, onClose, onSubmit, defaultValues, recordId, auftraegeList, kundenList, enablePhotoScan = true, enablePhotoLocation = true }: RechnungenDialogProps) {
  ...
  const REQUIRED_FIELDS = ['rechnungsnummer', 'auftrag', 'kunde', 'nettobetrag', 'mwst_satz', 'bruttobetrag', 'rechnungsdatum', 'faelligkeitsdatum', 'status_rechnung'] as const;
    ...
    try {
      ...
      if (file && (file.type.startsWith('image/') || file.type === 'application/pdf')) {
        try {
          ...
          setFields(prev => ({ ...prev, pdf_anhang: fileUrl }));
        } catch (uploadErr) {
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'kunde': (
      <div key="kunde" className="space-y-1.5">
        ...
        <Combobox
          id="kunde"
          placeholder="Wer bekommt die Rechnung?"
    ...
    'nettobetrag': (
      <div key="nettobetrag" className="space-y-1.5">
        <Label htmlFor="nettobetrag">{fieldLabel('rechnungen', 'nettobetrag')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="nettobetrag"
          type="number"
          step="any"
    ...
    'mwst_satz': (
      <div key="mwst_satz" className="space-y-1.5">
        <Label htmlFor="mwst_satz">{fieldLabel('rechnungen', 'mwst_satz')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('rechnungen', 'mwst_satz', 'mwst_19') ?? '19 %'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('rechnungen', 'mwst_satz', 'mwst_7') ?? '7 %'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('rechnungen', 'mwst_satz', 'mwst_0') ?? '0 %'}
          </button>
        </div>
        {showErrors && !fields.mwst_satz && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'bruttobetrag': (
      <div key="bruttobetrag" className="space-y-1.5">
        <Label htmlFor="bruttobetrag">{fieldLabel('rechnungen', 'bruttobetrag')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="bruttobetrag"
          type="number"
          step="any"
        ...
        />
        {showErrors && !fields.bruttobetrag && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'rechnungsdatum': (
      <div key="rechnungsdatum" className="space-y-1.5">
        ...
        <DatePicker
          id="rechnungsdatum"
          placeholder="Datum der Rechnungserstellung"
          mode="date"
    ...
    'status_rechnung': (
      <div key="status_rechnung" className="space-y-1.5">
        <Label htmlFor="status_rechnung">{fieldLabel('rechnungen', 'status_rechnung')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status_rechnung) === 'offen'}
            onClick={() => setFields(f => ({ ...f, status_rechnung: (lookupKey(f.status_rechnung) === 'offen' ? undefined : 'offen') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status_rechnung) === 'offen'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('rechnungen', 'status_rechnung', 'offen') ?? 'Offen'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status_rechnung) === 'bezahlt'}
            onClick={() => setFields(f => ({ ...f, status_rechnung: (lookupKey(f.status_rechnung) === 'bezahlt' ? undefined : 'bezahlt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status_rechnung) === 'bezahlt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('rechnungen', 'status_rechnung', 'bezahlt') ?? 'Bezahlt'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.status_rechnung) === 'ueberfaellig'}
            onClick={() => setFields(f => ({ ...f, status_rechnung: (lookupKey(f.status_rechnung) === 'ueberfaellig' ? undefined : 'ueberfaellig') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.status_rechnung) === 'ueberfaellig'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('rechnungen', 'status_rechnung', 'ueberfaellig') ?? 'Überfällig'}
          </button>
        </div>
        {showErrors && !fields.status_rechnung && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'pdf_anhang': (
      <div key="pdf_anhang" className="space-y-1.5">
        <Label htmlFor="pdf_anhang">{fieldLabel('rechnungen', 'pdf_anhang')}</Label>
        {fields.pdf_anhang ? (
          <div className="flex items-center gap-3 rounded-lg border p-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm truncate text-foreground">{fields.pdf_anhang.split("/").pop()}</p>
              <div className="flex gap-2 mt-1">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        setFields(f => ({ ...f, pdf_anhang: fileUrl }));
                ...
                <button
                  ...
                  onClick={() => setFields(f => ({ ...f, pdf_anhang: undefined }))}
                >
                  {t('fr_remove')}
                </button>
              </div>
  ...
  const FIELD_LABELS: Record<string, string> = {"rechnungsnummer": "Rechnungsnummer", "auftrag": "Auftrag", "kunde": "Kunde", "nettobetrag": "Nettobetrag (€)", "mwst_satz": "MwSt.-Satz", "bruttobetrag": "Bruttobetrag (€)", "rechnungsdatum": "Rechnungsdatum", "faelligkeitsdatum": "Fälligkeitsdatum", "status_rechnung": "Status", "pdf_anhang": "PDF-Anhang"};
  const CURRENCY_KEYS = new Set<string>(["nettobetrag", "bruttobetrag"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"auftrag": {"fahrzeug": "Fahrzeug", "kunde": "Kunde", "arbeitsbeschreibung": "Arbeitsbeschreibung", "wunschtermin": "Wunschtermin", "status": "Status", "prioritaet": "Priorität", "bemerkungen_auftrag": "Bemerkungen", "auftragsnummer": "Auftragsnummer"}, "kunde": {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "bemerkungen_kunde": "Bemerkungen"}};
