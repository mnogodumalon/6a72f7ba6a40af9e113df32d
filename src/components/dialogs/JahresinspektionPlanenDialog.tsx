/**
 * JahresinspektionPlanenDialog — pre-generated create/edit dialog for JahresinspektionPlanen.
 *
 ...
 * JahresinspektionPlanenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<JahresinspektionPlanen['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<JahresinspektionPlanenDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/JahresinspektionPlanen';
...
/** Widened prefill type for JahresinspektionPlanenDialog.defaultValues — see file header. */
export type JahresinspektionPlanenDialogDefaults = JahresinspektionPlanen['fields'];

interface JahresinspektionPlanenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: JahresinspektionPlanen['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: JahresinspektionPlanenDialogDefaults;
...
export function JahresinspektionPlanenDialog({ open, onClose, onSubmit, defaultValues, recordId, fahrzeugeList, enablePhotoScan = true, enablePhotoLocation = true }: JahresinspektionPlanenDialogProps) {
  const [fields, setFields] = useState<Partial<JahresinspektionPlanen['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['fahrzeug', 'wunschtermin_inspektion', 'arbeitsbeschreibung_inspektion'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<JahresinspektionPlanen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'jahresinspektion_planen');
      await onSubmit(clean as JahresinspektionPlanen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "fahrzeug": string | null, // Display name from Fahrzeuge (see <available-records>)\n  "wunschtermin_inspektion": string | null, // YYYY-MM-DDTHH:MM\n  "arbeitsbeschreibung_inspektion": string | null, // Arbeitsbeschreibung\n  "bemerkungen_inspektion": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["fahrzeug"]);
        ...
        return merged as Partial<JahresinspektionPlanen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('jahresinspektion_planen') })
    : t('new_entity', { entity: appLabel('jahresinspektion_planen') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'fahrzeug': (
      <div key="fahrzeug" className="space-y-1.5">
        <Label htmlFor="fahrzeug">{fieldLabel('jahresinspektion_planen', 'fahrzeug')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="fahrzeug"
          placeholder="Welches Fahrzeug inspizieren?"
    ...
    'wunschtermin_inspektion': (
      <div key="wunschtermin_inspektion" className="space-y-1.5">
        <Label htmlFor="wunschtermin_inspektion">{fieldLabel('jahresinspektion_planen', 'wunschtermin_inspektion')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="wunschtermin_inspektion"
          placeholder="Wann soll die Inspektion stattfinden?"
          mode="datetime"
          value={fields.wunschtermin_inspektion ?? null}
          onChange={v => setFields(f => ({ ...f, wunschtermin_inspektion: v ?? undefined }))}
          required
        />
        {showErrors && !fields.wunschtermin_inspektion && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'arbeitsbeschreibung_inspektion': (
      <div key="arbeitsbeschreibung_inspektion" className="space-y-1.5">
        <Label htmlFor="arbeitsbeschreibung_inspektion">{fieldLabel('jahresinspektion_planen', 'arbeitsbeschreibung_inspektion')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Textarea
          id="arbeitsbeschreibung_inspektion"
          placeholder="Was ist bei der Inspektion zu prüfen? Inspektions-Checkliste, Schwerpunkte..."
          value={fields.arbeitsbeschreibung_inspektion ?? ''}
          onChange={e => setFields(f => ({ ...f, arbeitsbeschreibung_inspektion: e.target.value }))}
          rows={3}
        />
        {showErrors && !fields.arbeitsbeschreibung_inspektion && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'bemerkungen_inspektion': (
      <div key="bemerkungen_inspektion" className="space-y-1.5">
        <Label htmlFor="bemerkungen_inspektion">{fieldLabel('jahresinspektion_planen', 'bemerkungen_inspektion')}</Label>
        <Textarea
          id="bemerkungen_inspektion"
          placeholder="Besonderheiten, Kundenhinweise, Vorbedingungen..."
  ...
  const FIELD_LABELS: Record<string, string> = {"fahrzeug": "Fahrzeug", "wunschtermin_inspektion": "Wunschtermin", "arbeitsbeschreibung_inspektion": "Arbeitsbeschreibung", "bemerkungen_inspektion": "Bemerkungen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"fahrzeug": {"kennzeichen": "Kennzeichen", "marke": "Marke", "modell": "Modell", "baujahr": "Baujahr", "fin": "Fahrzeugidentifikationsnummer (FIN)", "kilometerstand": "Kilometerstand (km)", "kunde": "Kunde"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.JAHRESINSPEKTION_PLANEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
