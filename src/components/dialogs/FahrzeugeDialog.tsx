...
export function FahrzeugeDialog({ open, onClose, onSubmit, defaultValues, recordId, kundenList, enablePhotoScan = true, enablePhotoLocation = true }: FahrzeugeDialogProps) {
  ...
  const REQUIRED_FIELDS = ['kennzeichen', 'marke', 'modell', 'kunde'] as const;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'baujahr': (
      <div key="baujahr" className="space-y-1.5">
        <Label htmlFor="baujahr">{fieldLabel('fahrzeuge', 'baujahr')}</Label>
        <Input
          id="baujahr"
          type="number"
          step="any"
    ...
    'fin': (
      <div key="fin" className="space-y-1.5">
        <Label htmlFor="fin">{fieldLabel('fahrzeuge', 'fin')}</Label>
        <Input
          id="fin"
          placeholder="z. B. WBADT41452G935814"
          value={fields.fin ?? ''}
    ...
    'kunde': (
      <div key="kunde" className="space-y-1.5">
        <Label htmlFor="kunde">{fieldLabel('fahrzeuge', 'kunde')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="kunde"
          placeholder="Fahrzeugeigentümer wählen"
  ...
  const FIELD_LABELS: Record<string, string> = {"kennzeichen": "Kennzeichen", "marke": "Marke", "modell": "Modell", "baujahr": "Baujahr", "fin": "Fahrzeugidentifikationsnummer (FIN)", "kilometerstand": "Kilometerstand (km)", "kunde": "Kunde"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"kunde": {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "bemerkungen_kunde": "Bemerkungen"}};
