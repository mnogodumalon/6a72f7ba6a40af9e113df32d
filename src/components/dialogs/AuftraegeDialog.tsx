...
import type { Auftraege, Fahrzeuge, Kunden, LookupValue } from '@/types/app';
...
export function AuftraegeDialog({ open, onClose, onSubmit, defaultValues, recordId, fahrzeugeList, kundenList, enablePhotoScan = true, enablePhotoLocation = true }: AuftraegeDialogProps) {
  ...
  const REQUIRED_FIELDS = ['fahrzeug', 'kunde', 'arbeitsbeschreibung', 'status', 'auftragsnummer'] as const;
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["fahrzeug", "kunde"]);
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'arbeitsbeschreibung': (
      <div key="arbeitsbeschreibung" className="space-y-1.5">
        <Label htmlFor="arbeitsbeschreibung">{fieldLabel('auftraege', 'arbeitsbeschreibung')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Textarea
          id="arbeitsbeschreibung"
          placeholder="Was genau muss repariert/gewartet werden? Beschwerden des Kunden, Symptome..."
          value={fields.arbeitsbeschreibung ?? ''}
          onChange={e => setFields(f => ({ ...f, arbeitsbeschreibung: e.target.value }))}
          rows={3}
        />
        {showErrors && !fields.arbeitsbeschreibung && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'wunschtermin': (
      <div key="wunschtermin" className="space-y-1.5">
        <Label htmlFor="wunschtermin">{fieldLabel('auftraege', 'wunschtermin')}</Label>
        <DatePicker
          id="wunschtermin"
          placeholder="Wann soll das Fahrzeug in der Werkstatt sein?"
          mode="datetime"
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('auftraege', 'status', 'in_bearbeitung') ?? 'In Bearbeitung'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('auftraege', 'status', 'abgeschlossen') ?? 'Abgeschlossen'}
          </button>
        </div>
    ...
    'prioritaet': (
      <div key="prioritaet" className="space-y-1.5">
        <Label htmlFor="prioritaet">{fieldLabel('auftraege', 'prioritaet')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('auftraege', 'prioritaet', 'niedrig') ?? 'Niedrig'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('auftraege', 'prioritaet', 'normal') ?? 'Normal'}
          </button>
          <button
            type="button"
            role="radio"
          ...
          >
            {lookupLabel('auftraege', 'prioritaet', 'hoch') ?? 'Hoch'}
          </button>
        </div>
      </div>
    ),
    'bemerkungen_auftrag': (
      <div key="bemerkungen_auftrag" className="space-y-1.5">
        <Label htmlFor="bemerkungen_auftrag">{fieldLabel('auftraege', 'bemerkungen_auftrag')}</Label>
        <Textarea
          id="bemerkungen_auftrag"
          placeholder="Zusätzliche Notizen, Besonderheiten, Material-Wünsche..."
  ...
  const FIELD_LABELS: Record<string, string> = {"fahrzeug": "Fahrzeug", "kunde": "Kunde", "arbeitsbeschreibung": "Arbeitsbeschreibung", "wunschtermin": "Wunschtermin", "status": "Status", "prioritaet": "Priorität", "bemerkungen_auftrag": "Bemerkungen", "auftragsnummer": "Auftragsnummer"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"fahrzeug": {"kennzeichen": "Kennzeichen", "marke": "Marke", "modell": "Modell", "baujahr": "Baujahr", "fin": "Fahrzeugidentifikationsnummer (FIN)", "kilometerstand": "Kilometerstand (km)", "kunde": "Kunde"}, "kunde": {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "bemerkungen_kunde": "Bemerkungen"}};
